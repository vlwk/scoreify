...
export async function signup(
  ...
  try {
    ...
    return {
      message: `Created user successfully with username ${username} and password ${passwordHash}`,
    };
  } catch (e) {
...
export async function login(
  ...
  try {
    ...
    return {
      message: `Signed in successfully with username ${username} and password_hash ${passwordHash}`,
    };
  } catch (e) {
    const error = e as Error;
    return { message: `Error: ${error.message}` };
...
export async function addTeams(
  ...
  const teamLines = data.teams_info
    .split("\n")
    .map((line) => line.trim())
  ...
  try {
    await sql.begin(async (trx) => {
      for (const line of teamLines) {
        const [team_name, registered_date, group_numberString] =
          line.split(" ");
        ...
        if (isNaN(group_number) || group_number < 1 || group_number > 2) {
          throw new Error(
            `Invalid group number for team ${team_name}: must be 1 or 2`,
          );
        }
        ...
        if (existingTeams.length > 0) {
          throw new Error(
            `Team ${team_name} already exists. Please choose another name.`,
          );
        }
        ...
        if (teamsInGroup[0].count >= 6) {
          throw new Error(
            `Group ${group_number} is full. Maximum 6 teams are allowed.`,
          );
        }
  ...
  try {
    await sql.begin(async (trx) => {
      ...
      for (const line of matchLines) {
        let [team1_name, team2_name, team1_score, team2_score] = line
          .trim()
          .split(" ");

        if (!team1_name || !team2_name || !team1_score || !team2_score) {
          throw new Error(
            "Invalid match format. Please ensure all fields are present.",
          );
        }
        ...
        if (existingMatches.length > 0) {
          throw new Error(
            `Match between ${team1_name} and ${team2_name} already exists.`,
          );
        }
        ...
        if (team1Data.length === 0 || team2Data.length === 0) {
          throw new Error(
            `One or both teams don't exist: ${team1_name}, ${team2_name}.`,
          );
        }
        ...
        if (team1Data[0].group_number !== team2Data[0].group_number) {
          throw new Error(
            `${team1_name} and ${team2_name} are not in the same group.`,
          );
        }
